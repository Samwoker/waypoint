...
pub async fn pause_destination(
    tenant: AuthenticatedTenant,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<impl IntoResponse, ApiError> {
    let destination = state
        .destination_service
        .pause_destination(tenant.tenant_id, id)
        .await
        .map_err(ApiError)?;
...
pub async fn resume_destination(
    tenant: AuthenticatedTenant,
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<impl IntoResponse, ApiError> {
    let destination = state
        .destination_service
        .resume_destination(tenant.tenant_id, id)
        .await
        .map_err(ApiError)?;
...
pub async fn test_destination(
    ...
    let result = state
        .destination_service
        .test_destination(tenant.tenant_id, id)
        .await
        .map_err(ApiError)?;
...
pub async fn get_destination_health(
    ...
    let health = state
        .destination_service
        .get_destination_health(tenant.tenant_id, id)
        .await
        .map_err(ApiError)?;
